---
layout: default
lang: la
title: Cursus Peracti
---

# DE CVRSVVS STVDIORVM CONFECTIS

Hic ea stvdia continentvr, qvae in lyceo atqve in vniversitate maxima cvm cvra excolvi.

## Ars Compvtatralis (COMP)

<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 mb-8">

    <blockquote style="margin: 0;">
        <span class="badge">COMP</span>
        <h3 style="margin-top: 0;">Systema Administrativvm Linvx</h3>
        <div class="course-details">
            <span class="course-tc">LINUX作業系統</span>
            <span class="course-en">Linux Operating System</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">COMP</span>
        <h3 style="margin-top: 0;">Ratio Programmatvm Scribendorvm</h3>
        <div class="course-details">
            <span class="course-tc">程序設計</span>
            <span class="course-en">Programming</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">COMP</span>
        <h3 style="margin-top: 0;">Logica Compvtatralis et Dispositio Partivm</h3>
        <div class="course-details">
            <span class="course-tc">數字邏輯與部件設計</span>
            <span class="course-en">Digital Logic and Component Design</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">COMP</span>
        <h3 style="margin-top: 0;">Strvctvra Datorvm</h3>
        <div class="course-details">
            <span class="course-tc">數據結構</span>
            <span class="course-en">Data Structure</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">COMP</span>
        <h3 style="margin-top: 0;">Principia Lingvarvm Programmatvrae</h3>
        <div class="course-details">
            <span class="course-tc">程序設計語言原理</span>
            <span class="course-en">Principles of Programming Languages</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">COMP</span>
        <h3 style="margin-top: 0;">Theoria Coetvvm et Graphismorvm</h3>
        <div class="course-details">
            <span class="course-tc">集合與圖論</span>
            <span class="course-en">Set Theory and Graph Theory</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">COMP</span>
        <h3 style="margin-top: 0;">Architectvra Compvtatralis</h3>
        <div class="course-details">
            <span class="course-tc">計算機組成與體系結構</span>
            <span class="course-en">Computer Organization and Architecture</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">COMP</span>
        <h3 style="margin-top: 0;">Principia Systematis Compvtatralis</h3>
        <div class="course-details">
            <span class="course-tc">計算機系統基礎</span>
            <span class="course-en">Principles of Computer System</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">COMP</span>
        <h3 style="margin-top: 0;">Mens Artificiosa</h3>
        <div class="course-details">
            <span class="course-tc">人工智能</span>
            <span class="course-en">Artificial Intelligence</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">COMP</span>
        <h3 style="margin-top: 0;">Designatio et Analysis Algorithmorvm</h3>
        <div class="course-details">
            <span class="course-tc">算法設計與分析</span>
            <span class="course-en">Algorithm Design and Analysis</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">COMP</span>
        <h3 style="margin-top: 0;">Fvndamenta Mathematica Mentis Artificiosae</h3>
        <div class="course-details">
            <span class="course-tc">人工智能的數學基礎</span>
            <span class="course-en">Mathematical Foundations of Artificial Intelligence</span>
        </div>
    </blockquote>

</div>

## Scientia Compvtatralis (CS)

<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 mb-8">

    <blockquote style="margin: 0;">
        <span class="badge">CS</span>
        <h3 style="margin-top: 0;">Compilatores</h3>
        <div class="course-details">
            <span class="course-tc">編譯</span>
            <span class="course-en">Compilers</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">CS</span>
        <h3 style="margin-top: 0;">Technologia Datorvm Magnorvm</h3>
        <div class="course-details">
            <span class="course-tc">大數據平臺技術</span>
            <span class="course-en">Technique of Big Data Platform</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">CS</span>
        <h3 style="margin-top: 0;">Systemata Discentiae Machinaris</h3>
        <div class="course-details">
            <span class="course-tc">機器學習系統</span>
            <span class="course-en">Machine Learning Systems</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">CS</span>
        <h3 style="margin-top: 0;">Graphica Compvtatralis</h3>
        <div class="course-details">
            <span class="course-tc">計算機圖形學</span>
            <span class="course-en">Computer Graphics</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">CS</span>
        <h3 style="margin-top: 0;">Cvratio Qvalitatis Programmatvrae</h3>
        <div class="course-details">
            <span class="course-tc">軟件質量保障與測試</span>
            <span class="course-en">Software Quality Assurance and Testing</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">CS</span>
        <h3 style="margin-top: 0;">Programmatvra Systematis</h3>
        <div class="course-details">
            <span class="course-tc">系統程序設計</span>
            <span class="course-en">System Programming</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">CS</span>
        <h3 style="margin-top: 0;">Systemata Operativa</h3>
        <div class="course-details">
            <span class="course-tc">作業系統</span>
            <span class="course-en">Operating Systems</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">CS</span>
        <h3 style="margin-top: 0;">Visio Compvtatralis</h3>
        <div class="course-details">
            <span class="course-tc">計算機視覺</span>
            <span class="course-en">Computer Vision</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">CS</span>
        <h3 style="margin-top: 0;">Tractatio Lingvae Natvralis</h3>
        <div class="course-details">
            <span class="course-tc">自然語言處理和知識表示</span>
            <span class="course-en">Natural Language Processing and Knowledge Representation</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">CS</span>
        <h3 style="margin-top: 0;">Compvtatio Nvbilaris</h3>
        <div class="course-details">
            <span class="course-tc">雲計算與虛擬化技術</span>
            <span class="course-en">Cloud Computing and Virtualization Technology</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">CS</span>
        <h3 style="margin-top: 0;">Discentia Profvnda</h3>
        <div class="course-details">
            <span class="course-tc">深度學習及其應用</span>
            <span class="course-en">Deep Learning: theory and applications</span>
        </div>
    </blockquote>

</div>

## Ingeniaria Programmatvrae (SOFT)

<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 mb-8">

    <blockquote style="margin: 0;">
        <span class="badge">SOFT</span>
        <h3 style="margin-top: 0;">Ingeniaria Programmatvrae</h3>
        <div class="course-details">
            <span class="course-tc">軟件工程</span>
            <span class="course-en">Software Engineering</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">SOFT</span>
        <h3 style="margin-top: 0;">Designatio Thecarvm Datorvm</h3>
        <div class="course-details">
            <span class="course-tc">數據庫設計</span>
            <span class="course-en">Database Design</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">SOFT</span>
        <h3 style="margin-top: 0;">Programmatvra Obiectis Directa</h3>
        <div class="course-details">
            <span class="course-tc">面向對象程序設計</span>
            <span class="course-en">Object-Oriented Programming</span>
        </div>
    </blockquote>

</div>

## Mathematica et Physica (MATH / PHYS)

<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 mb-8">

    <blockquote style="margin: 0;">
        <span class="badge">MATH</span>
        <h3 style="margin-top: 0;">Mathematica Provecta I</h3>
        <div class="course-details">
            <span class="course-tc">高等數學A(上)</span>
            <span class="course-en">Advanced Mathematics A I</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">MATH</span>
        <h3 style="margin-top: 0;">Mathematica Provecta II</h3>
        <div class="course-details">
            <span class="course-tc">高等數學A(下)</span>
            <span class="course-en">Advanced Mathematics A II</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">PHYS</span>
        <h3 style="margin-top: 0;">Physica Academica I</h3>
        <div class="course-details">
            <span class="course-tc">大學物理B(上)</span>
            <span class="course-en">College Physics B I</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">PHYS</span>
        <h3 style="margin-top: 0;">Physica Academica II</h3>
        <div class="course-details">
            <span class="course-tc">大學物理B(下)</span>
            <span class="course-en">College Physics B II</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">PHYS</span>
        <h3 style="margin-top: 0;">Experimenta Physicae Fvndamentalis</h3>
        <div class="course-details">
            <span class="course-tc">基礎物理實驗</span>
            <span class="course-en">Fundamental Physics Experiments</span>
        </div>
    </blockquote>

</div>

## Oeconomia (ECON)

<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 mb-8">

    <blockquote style="margin: 0;">
        <span class="badge">ECON</span>
        <h3 style="margin-top: 0;">Oeconomia Internationalis</h3>
        <div class="course-details">
            <span class="course-tc">國際經濟學</span>
            <span class="course-en">International Economics</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">ECON</span>
        <h3 style="margin-top: 0;">Oeconomia et Societas</h3>
        <div class="course-details">
            <span class="course-tc">經濟與社會</span>
            <span class="course-en">Economy and Society</span>
        </div>
    </blockquote>

</div>

## Lingvae et Hvmanitates (ENGL / HIST / FORE / ICES)

<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 mb-8">

    <blockquote style="margin: 0;">
        <span class="badge">ENGL</span>
        <h3 style="margin-top: 0;">Lingva Anglica Avdiovisvalis</h3>
        <div class="course-details">
            <span class="course-tc">英語視聽</span>
            <span class="course-en">Audiovisual English</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">ENGL</span>
        <h3 style="margin-top: 0;">Compositio Tractatvvm</h3>
        <div class="course-details">
            <span class="course-tc">英語論說文寫作</span>
            <span class="course-en">Essay Writing</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">ENGL</span>
        <h3 style="margin-top: 0;">Lingva Anglica Provecta</h3>
        <div class="course-details">
            <span class="course-tc">高級英語</span>
            <span class="course-en">Advanced English</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">FORE</span>
        <h3 style="margin-top: 0;">Lingva Germanica Fvndamentalis I</h3>
        <div class="course-details">
            <span class="course-tc">基礎德語Ⅰ</span>
            <span class="course-en">Basic German I</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">HIST</span>
        <h3 style="margin-top: 0;">Lingva Latina Fvndamentalis I</h3>
        <div class="course-details">
            <span class="course-tc">基礎拉丁語(上)</span>
            <span class="course-en">Basic Latin I</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">HIST</span>
        <h3 style="margin-top: 0;">Historia Civilizationis Asiae Orientalis</h3>
        <div class="course-details">
            <span class="course-tc">東亞文明史</span>
            <span class="course-en">History of East Asian Civilization</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">ICES</span>
        <h3 style="margin-top: 0;">Introdvctio in Lingvisticam</h3>
        <div class="course-details">
            <span class="course-tc">語言學概論</span>
            <span class="course-en">Introduction to Linguistics</span>
        </div>
    </blockquote>

</div>

## Artes, Cvltvra et Edvcatio Physica (FINE / GEEC / PEDU)

<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 mb-8">

    <blockquote style="margin: 0;">
        <span class="badge">FINE</span>
        <h3 style="margin-top: 0;">Aesthetica Mvsicae</h3>
        <div class="course-details">
            <span class="course-tc">中外音樂審美</span>
            <span class="course-en">Aesthetics of Chinese and Foreign Music</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">GEEC</span>
        <h3 style="margin-top: 0;">Cibvs Medicinalis et Cvltvra</h3>
        <div class="course-details">
            <span class="course-tc">藥膳與中國飲食文化</span>
            <span class="course-en">Medicinal Food and Chinese Dietetic Culture</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">PEDU</span>
        <h3 style="margin-top: 0;">Pvgilatvs Tai Chi XVI</h3>
        <div class="course-details">
            <span class="course-tc">十六式太極拳</span>
            <span class="course-en">16-Form Shadow Boxing</span>
        </div>
    </blockquote>

    <blockquote style="margin: 0;">
        <span class="badge">PEDU</span>
        <h3 style="margin-top: 0;">Pvgilatvs Tai Chi Stili Chen</h3>
        <div class="course-details">
            <span class="course-tc">陳式太極拳</span>
            <span class="course-en">Chen-Style Shadow Boxing</span>
        </div>
    </blockquote>

</div>
